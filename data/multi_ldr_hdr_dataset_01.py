...
class Multi_LDR_HDR_Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        ref_HR = read_img_01(HR_path, self.resize, self.size)

        # get LDR image
        LR_path = self.LDR_paths[index]
        in_LDR_1 = read_img_01(LR_path, self.resize, self.size)
        in_LDR_2 = read_img_01(LR_path.replace('1.', '2.'), self.resize, self.size)
        in_LDR_3 = read_img_01(LR_path.replace('1.', '3.'), self.resize, self.size)

        img_id = os.path.basename(LR_path).split('_')[0]
        exp_list = np.array(self.exp_dict[img_id]).astype(np.float32)
        in_HDR_1 = LDR2HDR_01(in_LDR_1, 2. ** exp_list[0])
        in_HDR_2 = LDR2HDR_01(in_LDR_2, 2. ** exp_list[1])
        in_HDR_3 = LDR2HDR_01(in_LDR_3, 2. ** exp_list[2])
