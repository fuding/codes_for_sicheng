...
class FirstBlock(nn.Module):
    def __init__(self, n_feats, exp=6, reduction=4, attention_block=None):
        ...
        self.b5 = InvertedResidual(n_feats, n_feats, expansion_factor=exp, reduction=reduction, attention_block=attention_block)
        ...
        self.c6 = BasicBlock(n_feats * 6, n_feats, 1, 1, 0)
...
class Block2(nn.Module):
    def __init__(self, n_feats, exp=6, reduction=4, attention_block=None):
        ...
        self.b4 = InvertedResidual(n_feats, n_feats, expansion_factor=exp, reduction=reduction,
                                   attention_block=attention_block)
        self.b5 = InvertedResidual(n_feats, n_feats, expansion_factor=exp, reduction=reduction,
                                   attention_block=attention_block)
        ...
        self.c5 = BasicBlock(n_feats * 6, n_feats, 1, 1, 0)
        self.c6 = BasicBlock(n_feats * 6, n_feats, 1, 1, 0)
    ...
    def forward(self, x):
        ...
        cat2 = torch.cat((c1, b1, x[1]), 1)
        c2 = self.c2(cat2)
        b2 = self.b2(c2)

        cat3 = torch.cat((c1, b1, b2, x[2]), 1)
        c3 = self.c3(cat3)
        b3 = self.b3(c3)

        cat4 = torch.cat((c1, b1, b2, b3, x[3]), 1)
        c4 = self.c4(cat4)
        b4 = self.b4(c4)

        cat5 = torch.cat((c1, b1, b2, b3, b4, x[4]), 1)
        c5 = self.c5(cat5)
        b5 = self.b5(c5)

        cat6 = torch.cat((c1, b1, b2, b3, b4, b5), 1)
        c6 = self.c6(cat6)
...
class Block3(nn.Module):
    def __init__(self, n_feats, exp=6, reduction=4, attention_block=None):
        ...
        self.c5 = BasicBlock(n_feats * 7, n_feats, 1, 1, 0)
        self.c6 = BasicBlock(n_feats * 6, n_feats, 1, 1, 0)
    ...
    def forward(self, x1, x2):
        ...
        cat2 = torch.cat((c1, b1, x1[1], x2[1]), 1)
        c2 = self.c2(cat2)
        b2 = self.b2(c2)

        cat3 = torch.cat((c1, b1, b2, x1[2], x2[2]), 1)
        c3 = self.c3(cat3)
        b3 = self.b3(c3)

        cat4 = torch.cat((c1, b1, b2, b3, x1[3], x2[3]), 1)
        c4 = self.c4(cat4)
        b4 = self.b4(c4)

        cat5 = torch.cat((c1, b1, b2, b3, b4, x1[4], x2[4]), 1)
        c5 = self.c5(cat5)
        b5 = self.b5(c5)
...
class Block4(nn.Module):
    def __init__(self, n_feats, exp=6, reduction=4, attention_block=None):
        ...
        self.c5 = BasicBlock(n_feats * 8, n_feats, 1, 1, 0)
        self.c6 = BasicBlock(n_feats * 6, n_feats, 1, 1, 0)

    def forward(self, x1, x2, x3):
        x_list = []

        cat1 = torch.cat((x3[0], x3[-1], x1[0], x2[0]), 1)
        c1 = self.c1(cat1)
        b1 = self.b1(c1)

        cat2 = torch.cat((c1, b1, x1[1], x2[1], x3[1]), 1)
        c2 = self.c2(cat2)
        b2 = self.b2(c2)

        cat3 = torch.cat((c1, b1, b2, x1[2], x2[2], x3[2]), 1)
        c3 = self.c3(cat3)
        b3 = self.b3(c3)

        cat4 = torch.cat((c1, b1, b2, b3, x1[3], x2[3], x3[3]), 1)
        c4 = self.c4(cat4)
        b4 = self.b4(c4)

        cat5 = torch.cat((c1, b1, b2, b3, b4, x1[4], x2[4], x3[4]), 1)
        c5 = self.c5(cat5)
        b5 = self.b5(c5)
...
class Block5(nn.Module):
    def __init__(self, n_feats, exp=6, reduction=4, attention_block=None):
        ...
        self.c1 = BasicBlock(n_feats * 5, n_feats, 1, 1, 0)
        self.c2 = BasicBlock(n_feats * 6, n_feats, 1, 1, 0)
        self.c3 = BasicBlock(n_feats * 7, n_feats, 1, 1, 0)
        self.c4 = BasicBlock(n_feats * 8, n_feats, 1, 1, 0)
        self.c5 = BasicBlock(n_feats * 9, n_feats, 1, 1, 0)
        self.c6 = BasicBlock(n_feats * 6, n_feats, 1, 1, 0)

    def forward(self, x1, x2, x3, x4):
        x_list = []

        cat1 = torch.cat((x4[0], x4[-1], x1[0], x2[0], x3[0]), 1)
        c1 = self.c1(cat1)
        b1 = self.b1(c1)

        cat2 = torch.cat((c1, b1, x1[1], x2[1], x3[1], x4[1]), 1)
        c2 = self.c2(cat2)
        b2 = self.b2(c2)

        cat3 = torch.cat((c1, b1, b2, x1[2], x2[2], x3[2], x4[2]), 1)
        c3 = self.c3(cat3)
        b3 = self.b3(c3)

        cat4 = torch.cat((c1, b1, b2, b3, x1[3], x2[3], x3[3], x4[3]), 1)
        c4 = self.c4(cat4)
        b4 = self.b4(c4)

        cat5 = torch.cat((c1, b1, b2, b3, b4, x1[4], x2[4], x3[4], x4[4]), 1)
        c5 = self.c5(cat5)
        b5 = self.b5(c5)
...
class DenseBlock(nn.Module):
    def __init__(self, n_feats=32, reduction=4, attention_blcok=None):
        ...
        self.block5 = Block5(n_feats, reduction=reduction, attention_block=ChannelAttention)

        self.fusion = nn.Conv2d(n_feats * 5, n_feats, 3, 1, 1, bias=False)
    ...
    def forward(self, x):
        ...
        x_list5 = self.block5(x_list1, x_list2, x_list3, x_list4)
        dense_out = torch.cat((x_list1[-1], x_list2[-1], x_list3[-1], x_list4[-1], x_list5[-1]), 1)
