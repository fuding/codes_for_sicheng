...
class Dense_6_CA_CA_4(nn.Module):
    def __init__(self, in_c=6, out_c=3, fc=64, n_feats=32, num_blocks=4, reduction=4):
        ...
        self.catattention1 = CatAttention(n_feats)
        self.catattention2 = CatAttention(fc*2)
        self.catattention3 = CatAttention(fc)
...
class CatAttention(nn.Module):
    ...
    def forward(self, x, x_cat):
        b, c, _, _ = x.size()
        y_1 = self.avg_pool_1(x).view(b, c)
        y_1 = self.se_1(y_1)
        y_1 = self.sigmoid_1(y_1).view(b, c, 1, 1)

        b, c, _, _ = x_cat.size()
        y_2 = self.avg_pool_2(x_cat).view(b, c)
        y_2 = self.se_2(y_2)
        y_2 = self.sigmoid_2(y_2).view(b, c, 1, 1)

        return x * y_1.expand_as(x) + x_cat * y_2.expand_as(x_cat)
