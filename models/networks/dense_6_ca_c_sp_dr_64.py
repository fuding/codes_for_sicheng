...
class Dense_6_CA_C_SP_DR_64(nn.Module):
    def __init__(self, in_c=6, out_c=3, fc=64, n_feats=32, num_blocks=4, reduction=4):
        super(Dense_6_CA_C_SP_DR_64, self).__init__()
        ...
        self.d1 = nn.Sequential(
            ...
            SubPixel(fc * 2, fc * 2, norm=True, act_type='relu')
        )

        self.d2 = nn.Sequential(
            nn.Conv2d(fc * 4, fc, 1, 1, 0),
            SubPixel(fc, fc, norm=True, act_type='relu')
        )
        self.d3 = nn.Sequential(
            nn.Conv2d(fc * 2, fc, 1, 1, 0),
            SubPixel(fc, fc, norm=True, act_type='relu')
        )
    ...
    def forward(self, in_LDRs, in_HDRs):
        ...
        e_3 = self.e_3(e_2)
        e_3_dr = self.dim_reduction(e_3)

        x_list1 = self.block1(e_3_dr)
