...
class AWSRN_SD_IA(nn.Module):
    # AWSRN_SD with input attention
    def __init__(self, in_c=6, out_c=3, fc=64, n_feats=16, ks=3, block_feats=128):
        super(AWSRN_SD_IA, self).__init__()
        ...
        self.attetion12 = nn.Sequential(
            BasicBlock(fc * 2, fc),
            BasicBlock(fc, fc, act_type='sigmoid'),
        )
        self.attetion23 = nn.Sequential(
            BasicBlock(fc * 2, fc),
            BasicBlock(fc, fc, act_type='sigmoid'),
        )
    ...
    def forward(self, in_LDRs, in_HDRs):
        ...
        att12 = torch.cat((e1_1, e1_1), dim=1)
        att12 = self.attetion12(att12)
        e1_1 = e1_1 * att12
        att23 = torch.cat((e2_1, e3_1), dim=1)
        att23 = self.attetion23(att23)
        e3_1 = e3_1 * att23

        e_cat = torch.cat((e1_1, e2_1, e3_1), 1)
        e_2 = self.e_2(e_cat)

        LFB_D = self.LFB_D(e_2)
