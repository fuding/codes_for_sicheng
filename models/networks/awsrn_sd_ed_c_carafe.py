...
class AWSRN_SD_ED_C_CARAFE(nn.Module):
    def __init__(self, in_c=6, out_c=3, fc=64, n_feats=16, ks=3, block_feats=128):
        super(AWSRN_SD_ED_C_CARAFE, self).__init__()
        ...
        self.channel_adapt_1 = nn.Sequential(
            nn.ReLU(inplace=True),
            nn.Conv2d(n_feats*2, fc*2, 3, 1, 1),
            nn.BatchNorm2d(fc * 2)
        )
        ...
        self.channel_adapt_2 = nn.Sequential(
            nn.ReLU(inplace=True),
            nn.Conv2d(fc * 4, fc , 3, 1, 1),
            nn.BatchNorm2d(fc)
        )
        ...
        self.channel_adapt_3 = nn.Sequential(
            nn.ReLU(inplace=True),
            nn.Conv2d(fc * 2, fc, 3, 1, 1),
            nn.BatchNorm2d(fc)
        )
    ...
    def forward(self, in_LDRs, in_HDRs):
        ...
        d1 = self.channel_adapt_1(d1)
        d1 = self.d1(d1)

        d2 = torch.cat((d1, e2_2), 1)
        d2 = self.channel_adapt_2(d2)
        d2 = self.d2(d2)

        d3 = torch.cat((d2, e2_1), 1)
        d3 = self.channel_adapt_3(d3)
        d3 = self.d3(d3)
