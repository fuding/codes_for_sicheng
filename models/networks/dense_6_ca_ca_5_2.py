...
class Dense_6_CA_CA_5_2(nn.Module):
    def __init__(self, in_c=6, out_c=3, fc=64, n_feats=32, num_blocks=4, reduction=4):
        ...
        self.dim_reduction = nn.Conv2d(fc, n_feats, 3, 1, 1)
        ...
        self.d1 = nn.Sequential(
            ...
            nn.ConvTranspose2d(fc, fc * 2, kernel_size=3, stride=2, padding=1, output_padding=1),
            nn.BatchNorm2d(fc * 2)
        )
        ...
        self.catattention1 = CatAttention(fc*2, fc)
    ...
    def forward(self, in_LDRs, in_HDRs):
        ...
        e_3 = self.e_3(e_2)
        dr = self.dim_reduction(e_3)

        x_list1 = self.block1(dr)
