...
class AWSRN_SD_ED_AW_ASPP(nn.Module):
    def __init__(self, in_c=6, out_c=3, fc=64, n_feats=16, ks=3, block_feats=128):
        super(AWSRN_SD_ED_AW_ASPP, self).__init__()
    ...
    def forward(self, in_LDRs, in_HDRs):
        ...
        ASPP = self.aspp(LFB_D)
        # AWMS = self.AWMS(LFB_D)

        d1 = self.awms_scale(ASPP) + self.e_3_scale(e_3)
        d1 = self.d1(d1)
