...
class AWSRN_ED_AW_MN_noBN(nn.Module):
    def __init__(self, in_c=6, out_c=3, fc=64, n_feats=32, ks=3, block_feats=128, num_blocks=4, reduction=4):
        ...
        for i in range(num_blocks):
            lfb.append(LFB_nobn(n_feats, wn, exp=6, reduction=reduction, attention_block=None))
...
class LFB_nobn(nn.Module):
    def __init__(
        self, n_feats, wn, exp=6, reduction=4, attention_block=None):
        super(LFB_nobn, self).__init__()
        self.b0 = InvertedResidual(n_feats, n_feats, expansion_factor=exp, attention_block=attention_block, reduction=reduction, norm=False)
        self.b1 = InvertedResidual(n_feats, n_feats, expansion_factor=exp, attention_block=attention_block, reduction=reduction, norm=False)
        self.b2 = InvertedResidual(n_feats, n_feats, expansion_factor=exp, attention_block=attention_block, reduction=reduction, norm=False)
        self.b3 = InvertedResidual(n_feats, n_feats, expansion_factor=exp, attention_block=attention_block, reduction=reduction, norm=False)
        self.reduction = nn.Conv2d(n_feats*4, n_feats, 3, padding=3//2)
